...
router.post(
  ...
  upload.single("file"),
  body(ZCourseMaterialCreate),
  instructorOwns,
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.file) throw new ErrorResponse(400, "File not uploaded");
      const url = req.file.path;
      const announcement = await coursematerialService.createCourseMaterial({
        ...req.body,
        url,
      });
      res
        .status(201)
        .send(new ResponseHandler(new ResponseData(201, announcement)));
    } catch (error) {
      next(error);
    }
  },
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const announcementList =
        await coursematerialService.getCourseCourseMaterial(course_id);
      res
        .status(200)
        .send(new ResponseHandler(new ResponseData(200, announcementList)));
    } catch (error) {
      next(error);
    }
  },
...
router.delete(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res
        .status(201)
        .send(new ResponseHandler(new ResponseData(201, "Deleted")));
    } catch (error) {
      next(error);
    }
  },
