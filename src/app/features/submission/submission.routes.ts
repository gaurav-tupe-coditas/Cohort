...
router.post(
  "/",
  permissionHandler("submit-assignment"),
  upload.single("file"),
  body(ZSubmisisonCreate),
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!req.file)
        throw new ErrorResponse(400, "Submission File not uploaded");
      ...
      res
        .status(201)
        .send(new ResponseHandler(new ResponseData(201, submission)));
    } catch (error) {
      next(error);
    }
  },
...
router.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const response =
        await submissionService.getSubmissionsForAssignment(assignment_id);
      res
        .status(200)
        .send(new ResponseHandler(new ResponseData(200, response)));
    } catch (error) {
...
router.patch(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res
        .status(200)
        .send(new ResponseHandler(new ResponseData(200, "Graded")));
    } catch (error) {
      next(error);
    }
  },
